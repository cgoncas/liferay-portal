		...
		return new AcceptLanguage() {
			...
			public Stream<Locale> getLocales() {
				...
				if (headerAcceptLanguage != null) {
					...
					localesList = Collections.list(locales);
				}
				else {
					localesList = Collections.emptyList();
				}
			...
			public Locale getPreferredLocale() {
				...
				Optional<CurrentUser> currentUserOptional = Optional.ofNullable(
					Try.fromFallible(
				...
				if (currentUserOptional.isPresent()) {
					...
					if (!Objects.isNull(currentUser.getWrappedModel()) &&
