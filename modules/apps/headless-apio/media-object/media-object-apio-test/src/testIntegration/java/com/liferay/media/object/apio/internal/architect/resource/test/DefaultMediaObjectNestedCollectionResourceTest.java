...
public class DefaultMediaObjectNestedCollectionResourceTest
	...
	public void testGetAdaptiveMedias() throws Exception {
		...
		AdaptiveMedia<AMImageProcessor> adaptiveMedia1000px =
			adaptiveMediaList.get(0);

		URI contentURL1000px = adaptiveMedia1000px.getURI();

		URL url1000px = new URL(TestPropsValues.PORTAL_URL + contentURL1000px);

		byte[] contentBytes1000px = IOUtils.toByteArray(url1000px.openStream());

		assertThat(
			contentBytes1000px.length,
			is(both(greaterThan(0)).and(lessThanOrEqualTo(bytes.length))));

		int imageWidthPixels = 512;

		assertThat(
			adaptiveMedia1000px.getValueOptional(
				AMImageAttribute.AM_IMAGE_ATTRIBUTE_HEIGHT).get(),
			is(Math.min(imageWidthPixels, 1000)));

		assertThat(
			adaptiveMedia1000px.getValueOptional(
				AMImageAttribute.AM_IMAGE_ATTRIBUTE_WIDTH).get(),
			is(Math.min(imageWidthPixels, 1000)));

		assertThat(
			adaptiveMedia1000px.getValueOptional(
				AMAttribute.getContentLengthAMAttribute()).get(),
			is(greaterThan(0L)));
		...
		AdaptiveMedia<AMImageProcessor> adaptiveMedia300px =
			adaptiveMediaList.get(1);

		URI contentURL300px = adaptiveMedia300px.getURI();

		byte[] contentBytes300px = IOUtils.toByteArray(
			new URL(TestPropsValues.PORTAL_URL + contentURL300px).openStream());

		assertThat(
			contentBytes300px.length,
			is(both(greaterThan(0)).and(lessThanOrEqualTo(bytes.length))));

		assertThat(
			adaptiveMedia300px.getValueOptional(
				AMImageAttribute.AM_IMAGE_ATTRIBUTE_HEIGHT).get(),
			is(Math.min(imageWidthPixels, 300)));
