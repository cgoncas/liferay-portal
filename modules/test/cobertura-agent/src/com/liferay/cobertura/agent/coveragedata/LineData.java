		...
		implements Comparable, CoverageData, HasBeenInstrumented, Serializable
{
		...
		try
		{
			if (jumps != null)
				for (int i = jumps.size() - 1; i >= 0; i--)
					ret += ((com.liferay.cobertura.agent.coveragedata.JumpData) jumps.get(i)).getNumberOfValidBranches();
			if (switches != null)
	...
	public int getNumberOfCoveredBranches()
	{
		...
		try
		{
			if (jumps != null)
				for (int i = jumps.size() - 1; i >= 0; i--)
					ret += ((com.liferay.cobertura.agent.coveragedata.JumpData) jumps.get(i)).getNumberOfCoveredBranches();
			if (switches != null)
				for (int i = switches.size() - 1; i >= 0; i--)
			...
			if (lineData.jumps != null)
				...
				else
				{
					for (int i = Math.min(this.jumps.size(), lineData.jumps.size()) - 1; i >= 0; i--)
						((com.liferay.cobertura.agent.coveragedata.JumpData) this.jumps.get(i)).merge((com.liferay.cobertura.agent.coveragedata.JumpData) lineData.jumps.get(i));
	...
	public String getConditionCoverage(int index) {
		...
		else if (branchData instanceof com.liferay.cobertura.agent.coveragedata.JumpData)
		{
			com.liferay.cobertura.agent.coveragedata.JumpData jumpData = (com.liferay.cobertura.agent.coveragedata.JumpData) branchData;
	...
	com.liferay.cobertura.agent.coveragedata.JumpData getJumpData(int jumpNumber)
	{
		lock.lock();
		try
		{
			...
			{
				for (int i = jumps.size(); i <= jumpNumber; jumps.add(new com.liferay.cobertura.agent.coveragedata.JumpData(i++)));
			}
			return (com.liferay.cobertura.agent.coveragedata.JumpData) jumps.get(jumpNumber);
		}
		finally
		{
			lock.unlock();
