	...
	implements Comparable<ClassData>, HasBeenInstrumented 
{
	 ...
	 */
	private Map<Integer, com.liferay.cobertura.agent.coveragedata.LineData> branches = new HashMap<Integer, com.liferay.cobertura.agent.coveragedata.LineData>();
	...
	public com.liferay.cobertura.agent.coveragedata.LineData addLine(int lineNumber, String methodName,
			String methodDescriptor)
	{
		lock.lock();
		try
		{
			com.liferay.cobertura.agent.coveragedata.LineData lineData = getLineData(lineNumber);
			if (lineData == null)
			{
				lineData = new com.liferay.cobertura.agent.coveragedata.LineData(lineNumber);
				// Each key is a line number in this class, stored as an Integer object.
				// Each value is information about the line, stored as a LineData object.
	...
	public double getBranchCoverageRate(String methodNameAndDescriptor)
	{
		...
		try
		{
			for (Iterator<com.liferay.cobertura.agent.coveragedata.LineData> iter = branches.values().iterator(); iter.hasNext();) {
				com.liferay.cobertura.agent.coveragedata.LineData next = (com.liferay.cobertura.agent.coveragedata.LineData) iter.next();
	...
	public com.liferay.cobertura.agent.coveragedata.LineData getLineCoverage(int lineNumber)
	{
		...
		try
		{
			...
			return (com.liferay.cobertura.agent.coveragedata.LineData) children.get(lineObject);
		}
		finally
		{
			lock.unlock();
	...
	private com.liferay.cobertura.agent.coveragedata.LineData getLineData(int lineNumber)
	{
		lock.lock();
		try
		{
			return (com.liferay.cobertura.agent.coveragedata.LineData)children.get(Integer.valueOf(lineNumber));
		}
		finally
		{
			lock.unlock();
	...
	public int getNumberOfValidBranches() 
	{
		...
		try
		{
			for (Iterator<com.liferay.cobertura.agent.coveragedata.LineData> i = branches.values().iterator();
				i.hasNext(); 
