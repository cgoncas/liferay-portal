...
public class JSLoaderModulesServlet extends HttpServlet
	...
	public class JSLoaderModule {
		...
		protected String generateMapsConfiguration(String configuration) {
			...
			for (int i = 0; i < namesJSONArray.length(); i++) {
				...
				mapsConfigurationJSONObject.put(subModuleName,  moduleRoot.concat(subModuleName));
			}
	...
	protected void service(
		...
		for (JSLoaderModule jsLoaderModule : jsLoaderModules) {
			...
			String mapsConfiguration = jsLoaderModule.getUnversionedMapsConfiguration();

			System.out.println(jsLoaderModule.getName() + " - Maps Configuration");
			System.out.println(mapsConfiguration);
